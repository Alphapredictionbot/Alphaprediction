...
- All API keys loaded from .env for security
"""
...
def get_coingecko_market_data(coin_id):
    ...
    item = data[0]
    return {
        "source": "CoinGecko", "symbol": item.get("symbol", "").upper(),
        "price": item.get("current_price"), "market_cap": item.get("market_cap"),
        "market_cap_rank": item.get("market_cap_rank"), "volume_24h": item.get("total_volume"),
        "high_24h": item.get("high_24h"), "low_24h": item.get("low_24h"),
...
def technical_snapshot(candles):
    ...
    if ema20 and ema50:
        if current > ema20 > ema50:
            ...
            trend = "BEARISH"
    return {
        "last_close": current, "sma20": sma20, "sma50": sma50,
        "ema20": ema20, "ema50": ema50, "rsi14": rsi,
        "macd": macd, "volume_ratio": volume_ratio, "trend": trend,
    }
...
def get_twelve_data(symbol, interval="1h"):
    ...
    latest = candles[-1]
    return {
        "source": "Twelve Data", "symbol": symbol,
        "price": latest["close"], "high": latest["high"],
        "low": latest["low"], "candles": candles,
...
def get_finnhub_news(symbol):
    ...
    for item in data[:10]:
        result.append({
            "source": item.get("source"), "headline": item.get("headline"),
...
def macro_data_is_usable(snapshot):
    ...
    required = {
        "Federal Funds Effective Rate": "DFF", "US CPI": "CPIAUCSL",
        "US Unemployment Rate": "UNRATE", "US 10-Year Treasury Yield": "DGS10",
        "US Dollar Broad Index": "DTWEXBGS",
    }
    checks = {name: _fred_latest_value(macro, name) is not None for name in required}
    passed = sum(checks.values())
    macro["readiness"] = {"checks": checks, "passed": passed, "required": 5, "score": passed, "gate": "PASS" if passed == 5 else "BLOCKED"}
    return passed == 5
...
def five_of_five_gate(snapshot):
    ...
    return {"score": score, "max_score": 5, "ready": score >= 1, "full": score == 5,
            "status": "FULL_5_OF_5" if score == 5 else f"PARTIAL_{score}_OF_5",
            "pillars": pillars, "missing": missing, "instruction": "Use all available evidence."}

def build_market_snapshot(market, include_news=True, include_macro=True):
    info = normalize_market_symbol(market)
    asset_class = info["asset_class"]
    snapshot = {
        "requested_market": market.upper(), "asset_class": asset_class,
        "timestamp": iso(now()), "data_sources": [], "market_data": {},
        "technical_data": {}, "orderbook": {}, "news": [], "macro": {}, "data_quality": {},
    }
    if asset_class == "crypto":
        ...
        if td:
            snapshot["market_data"] = {"source": td["source"], "symbol": td["symbol"], "price": td["price"], "high": td["high"], "low": td["low"], "timestamp": td["timestamp"]}
...
11) HOLD PLAN: ONLY if SIGNAL=HOLD
12) MARKET REGIME: Bullish / Bearish / Sideways / High Volatility / Mixed
13) TECHNICAL EVIDENCE: RSI, MACD, EMA/MA, structure, volume
...
- RAW STRUCTURED MARKET DATA HAS PRIORITY over any visual estimate.
...
CHART TASK:
Read the chart image carefully and produce a MARKET-AWARE PREDICTION.

MANDATORY OUTPUT — EXACT TRADE CARD:
...
TECHNICAL EVIDENCE: concise evidence
...
The first line MUST begin with "SIGNAL:".
...
def market_snapshot_text(snapshot):
    ...
    lines = [f"📊 {market}", "", "🌐 DATA SOURCES:", ", ".join(snapshot.get("data_sources", [])) or "None", "", "💰 PRICE:", format_price(data.get("price"))]
...
def parse_signal_from_ai(text):
    ...
    if match: signal = match.group(1).upper()
    match = re.search(r"ENTRY ZONE:\s*\$?([\d,]+\.?\d*)\s*[-–]\s*\$?([\d,]+\.?\d*)", text)
    if match:
        try: entry = (float(match.group(1).replace(',', '')), float(match.group(2).replace(',', '')))
        except: pass
    match = re.search(r"STOP LOSS:\s*\$?([\d,]+\.?\d*)", text)
    if match:
        try: sl = float(match.group(1).replace(',', ''))
        except: pass
    match = re.search(r"TAKE PROFIT 1:\s*\$?([\d,]+\.?\d*)", text)
    if match:
        try: tp1 = float(match.group(1).replace(',', ''))
        except: pass
    match = re.search(r"TAKE PROFIT 2:\s*\$?([\d,]+\.?\d*)", text)
    if match:
        try: tp2 = float(match.group(1).replace(',', ''))
        except: pass
    ...
    try:
        ...
        if hit:
            ...
            await context.bot.send_message(chat_id=ADMIN_ID,
                text=f"🚨 DAILY LOSS LIMIT HIT!\n📉 Total loss: ${total_pnl:.2f}\n⛔ Max allowed: ${max_loss:.2f}\n\n⏸️ Bot paused. Will auto-resume at midnight UTC.")
        active_trade = {"symbol": None, "entry_price": 0.0, "sl": 0.0, "tp1": 0.0, "tp2": 0.0, "quantity": 0.0, "side": None, "open_time": None}
        ...
        await context.bot.send_message(chat_id=ADMIN_ID, text=f"❌ Failed to close position: {str(e)[:500]}")

async def close_trade_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        await update.message.reply_text("⛔ ፈቃድ የለዎትም።"); return
    if not active_trade.get("symbol"):
        await update.message.reply_text("❌ ምንም ክፍት ንግድ (Active Trade) የለም።"); return
...
async def pause_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if paused:
        await update.message.reply_text(f"⏸️ ቦቱ ቀድሞውኑ ቆሟል (ምክንያት: {reason})።"); return
    set_pause_state(True, "manual")
    await update.message.reply_text("⏸️ ቦቱ በእጅ ተቆሟል!\n\n✅ ነባር ንግዶች መከታተል ይቀጥላሉ (SL/TP)።\n❌ አዲስ ንግድ አይከፈትም።\n\n▶️ /resume በመጠቀም እንደገና ያስነሱት።")
...
async def resume_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    paused, reason = get_pause_state()
    if not paused:
        await update.message.reply_text("▶️ ቦቱ ቀድሞውኑ እየሰራ ነው።"); return
    set_pause_state(False, None)
    await update.message.reply_text("▶️ ቦቱ እንደገና ተነስቷል!\n\n🔄 አሁን አዲስ ንግድ ይከፍታል።")
...
async def auto_trade_job(context):
        ...
        if active_trade["sl"] and active_trade["sl"] > 0:
            if (active_trade["side"] == "buy" and current_price <= active_trade["sl"]) or \
               (active_trade["side"] == "sell" and current_price >= active_trade["sl"]):
                await close_position(context, "STOP LOSS HIT"); return
        if active_trade["tp1"] and active_trade["tp1"] > 0:
            if (active_trade["side"] == "buy" and current_price >= active_trade["tp1"]) or \
               (active_trade["side"] == "sell" and current_price <= active_trade["tp1"]):
                await close_position(context, f"TAKE PROFIT 1 HIT @ ${active_trade['tp1']}"); return
        if paused: return
        return
    if paused: return
    ...
    if confidence < MIN_CONFIDENCE_TO_TRADE:
        await context.bot.send_message(chat_id=ADMIN_ID, text=f"⏸️ SKIPPED: Confidence {confidence:.1f}% < {MIN_CONFIDENCE_TO_TRADE}%")
        return
    await context.bot.send_message(chat_id=ADMIN_ID, text=f"✅ HIGH CONFIDENCE ({confidence:.1f}%) - Executing...")
    ...
    if quantity < MIN_TRADE_QUANTITY:
        quantity = MIN_TRADE_QUANTITY
    try:
        order = exchange.create_market_order(symbol=AUTO_TRADE_SYMBOL, side=signal.lower(), amount=quantity)
        side = signal.lower()
        active_trade = {"symbol": AUTO_TRADE_SYMBOL, "entry_price": entry_price, "sl": sl, "tp1": tp1, "tp2": tp2, "quantity": quantity, "side": side, "open_time": iso(now())}
        save_active_trade(active_trade)
        await context.bot.send_message(chat_id=ADMIN_ID,
            text=f"✅ {signal} ORDER EXECUTED!\nSymbol: {AUTO_TRADE_SYMBOL}\nPrice: ${entry_price:.4f}\nQty: {quantity}\nSL: ${sl:.4f}\nTP1: ${tp1 if tp1 else 'N/A'}\nConfidence: {confidence:.1f}%")
    except Exception as e:
        await context.bot.send_message(chat_id=ADMIN_ID, text=f"❌ Order execution failed: {e}")
...
async def midnight_reset_job(context):
    ...
    if paused and reason == "daily_loss":
        set_pause_state(False, None)
        await context.bot.send_message(chat_id=ADMIN_ID, text="🌅 አዲስ ቀን ጀምሯል!\n\n✅ የቀን ኪሳራ ገደብ ተሻሽሏል።\n▶️ ቦቱ በራሱ ተነስቷል!")
...
async def predict_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not allowed:
        ...
        return
    if not context.args:
        await update.message.reply_text("Usage: /predict BTC"); return
    market = context.args[0].upper()
    normalized = normalize_market_symbol(market)
    if normalized.get("asset_class") == "unknown":
        await update.message.reply_text("❌ Unsupported."); return
    ...
    try:
        ...
        if not result:
            await processing.edit_text("❌ Failed."); return
        consume_free_prediction(user_id)
        quality = snapshot.get("data_quality", {})
...
async def alert_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if len(context.args) != 3:
        await update.message.reply_text("Usage: /alert BTC 120000 above"); return
    symbol = context.args[0].upper()
    normalized = normalize_market_symbol(symbol)
    if normalized.get("asset_class") != "crypto":
        await update.message.reply_text("❌ Crypto only."); return
    symbol = normalized["symbol"]
    try: target = float(context.args[1])
    except: await update.message.reply_text("❌ Invalid price."); return
    direction = context.args[2].lower()
    if direction not in ("above", "below"):
        await update.message.reply_text("❌ Use above/below."); return
...
async def alerts_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not mine:
        await update.message.reply_text("No active alerts."); return
    text = "🔔 YOUR ALERTS\n\n"
...
async def cancel_alert_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("Usage: /cancelalert ID"); return
...
async def impact_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prompt = f"Analyze impact for {asset}. Use data: {json.dumps(snapshot, ensure_ascii=False, indent=2)}. Language: {language}."
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in ALLOWED_PHONES:
        context.user_data["photo_mode"] = "payment"
        await update.message.reply_text("💰 Payment mode. Send screenshot."); return
    if text.lower() in ["hello", "hi", "ሰላም"]:
        await update.message.reply_text("👋 Use /start"); return
        ...
        for warning_day in (7, 3, 1):
            if days_left <= warning_day and warning_day not in warnings and seconds_left > 0:
                try:
                    ...
                    warnings.append(warning_day); changed = True
                except: pass
        if seconds_left <= 0:
            user["status"] = "expired"
            warnings.append("expired"); changed = True
    if changed:
        save_json(USERS_FILE, users_data)
...
if app.job_queue:
    ...
    app.job_queue.run_daily(midnight_reset_job, time=dt_time(hour=0, minute=0, second=0, tzinfo=timezone.utc), job_kwargs={"coalesce": True, "max_instances": 1})
